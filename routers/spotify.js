...
var clientAuthAccessToken =

    spotify.route("/authorize/user")
        .get(async (req, res) => {
            ...
            if (req.query.refresh_token) { // USER WANTS TO REFRESH TOKEN
            ...
            if (req.query.code) { // USER WANTS TO EXCHANGE AUTH CODE FOR TOKENS
                const result = await fetch("https://accounts.spotify.com/api/token", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/x-www-form-urlencoded"
                ...
                res.redirect(`http://localhost:3000/artists?access_token=${result.access_token}&refresh_token=${result.refresh_token}&expires_in=${result.expires_in}`)
            }
            else { // USER IS REQUESTING AUTH CODE
                var scopes = 'user-read-private user-read-email user-top-read';
                res.redirect('https://accounts.spotify.com/authorize' +
                    '?response_type=code' +
                    '&client_id=' + 'bc453c61d93d4492b22bb580cf33db9f' +
...
spotify.route("/authorize/client")
    .get(async (req, res) => {
        res.setHeader("Access-Control-Allow-Origin", "*")
        console.log("client id", process.env.SPOTIFY_CLIENT_ID, process.env.SPOTIFY_CLIENT_SECRET)
        var clientCredentials = await fetch("https://accounts.spotify.com/api/token", {
            method: "POST",
...
spotify.route("/search")
    .get(async (req, res) => {
        ...
        const accessToken = (await fetch("http://localhost:4000/spotify/authorize/client").then(response => response.json())).access_token
        console.log("heres the token!!!!!!!!!!!!!!!!", accessToken)
        spotifyApiClient.setAccessToken(accessToken)
        const artists = await spotifyApiClient.searchArtists(searchValue, { limit: 50 })
        //res.send("hello")
        res.json(artists.body)

    })

spotify.route("/topartists")
    .get(async (req, res) => {
...
spotify.route("/followedartists")
    .get(async (req, res) => {
        ...
        try {
            ...
            if (databaseArtists.length == 0) { res.json({ artists: [] }); return } // if there aren't any artists in the DB then we send back an empty array and end the endpoint execution
            var artists = await spotifyApiClient.getArtists(databaseArtists.map(databaseArtist => databaseArtist.spotifyId))
